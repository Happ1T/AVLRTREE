...
public class MyPars {
    ...
    public static NodeForAVL parse(String exp) {
        int count = 0;
        ListNew<String> queue = new MyLinkedList();
    ...
    private static NodeForAVL parse(ListNew<String> queue) {
        NodeForAVL root = null;
        if (queue.getFirst() == "(") {
            queue.removeFirst();
            if (queue.getFirst() != ")") {
                root = new NodeForAVL(Integer.parseInt(queue.getFirst()));
                queue.removeFirst();
                if (queue.getLast() == ")") {
                ...
                int count = 0;
                boolean isLeft = true;
                ListNew q = new MyLinkedList<String>();
